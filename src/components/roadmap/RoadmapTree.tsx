...
//   onNodeClick,
// }) => {
//   const [nodeElements, setNodeElements] = useState<{ [key: string]: HTMLElement | null }>({});
...
//   const getNodeCenter = (element: HTMLElement) => {
//     const rect = element.getBoundingClientRect();
//     const containerRect = element.parentElement?.getBoundingClientRect() || rect;
//     return {
...
//           const startElement = nodeElements[depId];
//           const endElement = nodeElements[node.id];
          
//           if (startElement && endElement) {
//             const start = getNodeCenter(startElement);
//             const end = getNodeCenter(endElement);
...
//       >
//         <h3 className="font-bold text-gray-900 mb-2">How to use this roadmap</h3>
...
//           </li>
//           <li className="flex items-center mt-4 text-indigo-600">
//             <span>Drag nodes to rearrange the roadmap</span>
//           </li>
//         </ul>
//       </motion.div>
//     </div>
...
//   // Calculate tree-like positions based on dependencies
//   const calculateTreePositions = () => {
//     const positions: { [key: string]: { x: number; y: number } } = {};
//     const levelSpacing = 250; // Vertical spacing between levels
//     const siblingSpacing = 500; // Horizontal spacing between sibling nodes

//     const placeNode = (nodeId: string, level: number, xOffset: number) => {
//       if (positions[nodeId]) return; // Node already placed
...
//       // Place children
//       const children = nodes.filter((n) => n.dependencies.includes(nodeId));
//       const childXOffsetStart = x - (children.length - 1) * siblingSpacing / 2;

//       children.forEach((child, index) => {
//         placeNode(child.id, level + 1, childXOffsetStart + index * siblingSpacing);
//       });
//     };

//     // Start placing nodes from the root nodes (nodes with no dependencies)
//     const rootNodes = nodes.filter((node) => node.dependencies.length === 0);
//     rootNodes.forEach((rootNode, index) => {
//       placeNode(rootNode.id, 0, index * siblingSpacing * 2); // Spread root nodes horizontally
//     });
...
//     console.log('Calculated Positions:', positions); // Debugging log
//     setNodePositions(positions);
//   }, [nodes]);

//   return (
//     <div className="relative w-full min-h-[800px] flex justify-center items-center">
//       {/* Render connectors */}
//       {nodes.map((node) =>
//         node.dependencies.map((depId) => {
...
//             className="absolute"
//             style={{
//               left: `calc(50% + ${nodePositions[node.id]?.x || 0}px)`, // Center horizontally
//               top: `calc(50% + ${nodePositions[node.id]?.y || 0}px)`, // Center vertically
//             }}
//           >
//             <RoadmapNode
//               node={node}
